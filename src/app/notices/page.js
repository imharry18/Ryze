    ...
    <div className="relative w-full mb-6 break-inside-avoid animate-fadeIn">
      <div className={`rounded-2xl border shadow-lg overflow-hidden relative ${themeClass}`}>
          ...
          <div className="p-6">
            ...
            {/* In the new upload hook, we passed 'details' inside extraData, 
                and our API saved it to the root of the Mongo document. */}
            <div className="mb-6">
...
export default function NoticesPage() {
  ...
  useEffect(() => {
    async function fetchNotices() {
        try {
            const res = await fetch("/api/posts?postType=notice");
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                    {filteredNotices.map(post => (
                        <NoticeCard key={post._id} post={post} />
                    ))}
                </div>
            )}
        </div>
