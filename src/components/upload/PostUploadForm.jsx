...
export default function PostUploadForm({ onClose }) {
  ...
  const handleFile = (e) => {
    ...
    if (!f) return;
    if (!f.type.startsWith("image/")) return alert("Please pick an image.");
  ...
  const startUpload = async () => {
    ...
    try {
      await upload({ 
        uid: user.id, // NextAuth ID (Postgres)
        file, 
        mediaType: "image", 
        caption, 
        location, 
    ...
    <div className="flex flex-col md:flex-row gap-6 p-4 min-h-[400px]">
      <div className="w-full md:w-1/2 bg-black/60 rounded-xl border border-white/10 flex items-center justify-center overflow-hidden relative group">
        {previewURL ? (
          <div className="relative w-full h-full flex items-center justify-center bg-black">
             ...
             <button 
               onClick={() => { setFile(null); setPreviewURL(""); fileRef.current.value = ""; }}
      ...
      <div className="w-full md:w-1/2 flex flex-col">
        ...
        <div className="space-y-4 flex-1">
            <textarea value={caption} onChange={(e)=>setCaption(e.target.value)} placeholder="Write a caption..." className="w-full p-3 bg-white/5 border border-white/10 rounded-lg h-24 resize-none focus:outline-none focus:border-blue-500 text-white transition" />
            <div className="relative">
                <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                <Input value={location} onChange={(e)=>setLocation(e.target.value)} placeholder="Location" className="pl-9 bg-white/5 border-white/10" />
            </div>
        </div>
        ...
        <div className="mt-6 pt-4 border-t border-white/10">
          {uploading && <div className="text-blue-400 text-xs mb-2">Uploading: {progress}%</div>}
